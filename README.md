...
- **带读陪练（Tutor）**：先出 RoadMap 看清全程，再逐问带读——AI 按「合格回答应包含」苏格拉底式追问并判定理解；途中可 `问：…` 直接提问答疑；想看图可输入 `画图：请求流程` 这类主题，按需生成局部图；动手实验可选（`跳过` 即可），毕业可贴 3 分钟讲解稿验证「能讲」
- **架构图 / 局部图**：AI 概览页可一键生成 8–12 节点的总体架构图（Mermaid），带读步骤也会在需要时配局部小图，帮新手先建立画面再读代码
...
单页包含：静态报告（免费）→ AI 概览（含工具调用轨迹与「架构总览图」）→ 边读边问 → 学习进度勾选清单 → **带读陪练**。
带读陪练（Tab 05）把剧本搬进浏览器：点「开始带读」自动复用/生成剧本，先在页面上方显示
RoadMap 总览，再进入会话逐步带读；「重新生成剧本」会先读带读记忆，跳过已掌握内容、加深
薄弱点（记忆驱动的自适应路线）；学习途中随时输入 `问：…` 直接提问答疑，动手实验可
`跳过`，毕业可用 3 分钟讲解稿验证「能讲」。想理解某段链路时输入 `画图：请求流程` 这类主题，
AI 会按需生成一张局部图（flowchart/时序）显示在会话上方。带读结果会自动记入
...
│   ├── diagram.py         # Mermaid 图渲染抽象（架构总览图 / 步骤局部图）
